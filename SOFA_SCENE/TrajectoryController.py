...
class CircleTrajectory(Sofa.Core.Controller):
        """ 
        FR :
        Génère un trajectoire circulaire à l'effecteur (goal point)
            INPUT : 
            rayon = rayon du cercle de la trajectoire (en mm)
        ...
        EN :
        Generate a circular trajectory to the end-effector (goal point)
             INPUT:
             radius = radius of the circle of the trajectory (in mm)
             nb_iter = number of iterations, of calculation steps, to perform a complete circle (gives the speed at which the circle trajectory will be executed)
             node = node on is the object that will make a circle
             name = name of the object that contains the position to vary
             circle_height = height of the circle drawn by the effector (along the z axis here)
             module = variable stiff which contains all the data of the robot
             axis = variable which gives the plane in which we draw the circle ([1,1,0] = x,y, [1,0,1] = x,z)
...
class SquareTrajectory(Sofa.Core.Controller):
    ...
    EN :
    Generates a square trajectory to the effector
         INPUT:
         length = length of one side
         nb_iter = nb of iteration to produce one side of the square
         node = node that contains the object that will make a square
         name = name of the object that contains the position that will draw a square
         square_height = height of the square drawn by the effector (along the z axis here)
         module = variable stiff which contains all the data of the robot
         axis = variable which gives the plane in which we draw the square ([1,1,0] = x,y, [1,0,1] = x,z)
...
class LineTrajectory(Sofa.Core.Controller):
    ...
    Génère un trajectoire linéaire 
        ...
        nb_iter = nb d'itération pour réaliser un coté du carré 
        node = noeud sur se trouve l'objet qui va effectuer une trajectoire linéaire
        name = nom de l'objet qui contient la position à faire varier

    EN :
    Generates a linear trajectory
         INPUT:
         p_begin = coordinate of the point at the ends of the segment start
         p_end = coordinate of the point at the extremities of the end of the segment
         nb_iter = nb of iteration to produce one side of the square
         node = node on is the object that will perform a linear trajectory
         name = name of the object that contains the position to vary
